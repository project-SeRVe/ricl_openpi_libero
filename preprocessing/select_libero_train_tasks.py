...
def select_and_move_train_tasks(
    source_dir: Path,
    target_dir: Path,
    num_tasks: int,
    shuffle: bool,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Select N LIBERO task folders from source and move them to target for training dataset construction."
        )
    )
    parser.add_argument(
        "--source_dir",
        ...
        help="Source folder under preprocessing that contains all processed LIBERO task folders.",
    )
    parser.add_argument(
        "--target_dir",
        type=str,
        default="libero_collected_demos_training",
        help="Target folder under preprocessing where selected training task folders are moved.",
    )
    parser.add_argument("--num_tasks", type=int, required=True, help="Number of task folders to move.")
    parser.add_argument(
        "--shuffle",
        action="store_true",
    ...
    selected, remaining = select_and_move_train_tasks(
